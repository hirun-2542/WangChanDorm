...
- **ต้องอยู่นอก Cloudflare Access**: `/slips/*` ถูกใส่ใน `assets.run_worker_first` แล้ว และยังเป็น route สาธารณะที่หน้า "รอตรวจ" ของเจ้าของใช้แสดงรูปสลิป (ดูหัวข้อ Access ด้านล่าง) — การตรวจสลิปไม่ต้องพึ่งรูปที่เปิดสาธารณะอีกต่อไป เพราะแนบไฟล์รูปไปกับ request เลย
...
4. แยก path ที่ต้องเปิดสาธารณะออกจาก Access: `/webhook/*` (LINE เรียกเข้ามา), `/qr/*` และ `/invoices/*` (ลิงก์รูป QR และใบแจ้งหนี้ PDF ที่ฝังในข้อความ LINE), `/slips/*` (route รูปสลิปที่หน้า "รอตรวจ" ของเจ้าของใช้แสดง — การตรวจสลิปไม่ได้พึ่งรูปที่เปิดสาธารณะแล้ว) และ `/register` + `/api/register/*` (หน้าลงทะเบียนผู้เช่าที่เปิดจากในแอป LINE ก่อนผู้ใช้ล็อกอินได้) — เช่นใช้ Bypass policy ตาม path

`/slips/*` ยังเปิดสาธารณะอยู่ เพราะเป็น route ที่หน้า "รอตรวจ" ของเจ้าของใช้แสดงรูปสลิป และความลับของรูปคือคีย์สุ่ม 128-bit ที่เดาไม่ได้ ไม่ใช่การบังคับล็อกอิน; เหตุผลเดิมที่ว่า "ต้องเปิดไว้ให้ SlipOK ดึงรูปไปตรวจ" ตกไปแล้ว เพราะไคลแอนต์แนบไฟล์รูปไปกับ request หา SlipOK เอง รูปจึงไม่ต้องเข้าถึงจากอินเทอร์เน็ต และการปิดบิลอัตโนมัติไม่ขึ้นกับว่า `/slips/*` เปิดอยู่หรือไม่ — ส่วน `/qr/*` และ `/invoices/*` ยังต้องสาธารณะด้วยเหตุผลเดิมคือลิงก์ถูกส่งออกไปในข้อความ LINE ของผู้เช่า
